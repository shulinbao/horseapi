...
const translations = {
  ...
  'page_range': {
    'zh-Hant': '第 ${page.currentStart} - ${page.currentEnd} 條，共 ${tokens.length} 條',
    'zh': '第 ${page.currentStart} - ${page.currentEnd} 条，共 ${tokens.length} 条',
    'en': 'Page ${page.currentStart} - ${page.currentEnd}, total ${tokens.length} items'
  },
  "confirm": {
    "zh-Hant": "確定",
    "zh": "确定",
...
const TokensTable = () => {
  ...
  const columns = [
    ...
    render: (text, record, index) => {
      ...
      let chatsArray = []
      let chatLink = '/chat?settings={"key":"{key}"}';
            ...
            for (let i = 0; i < chats.length; i++) {
              ...
              for (let key in chats[i]) {
                if (chats[i].hasOwnProperty(key)) {
                  ...
                  chat.name = key;
                  chat.onClick = () => {
					let customLink = `/chat?settings={"key":"{key}"}`
					onOpenLink(key, customLink, record);
                  }
                }
              }
...
return (
  <>
    ...
    <Table
      ...
      getTranslation('page_range', userLanguage).replace('${page.currentStart}', page.currentStart)
        .replace('${page.currentEnd}', page.currentEnd)
        .replace('${tokens.length}', tokenCount),
