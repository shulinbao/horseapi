...
func GetRedemptionById(id int) (*Redemption, error) {
	if id == 0 {
		return nil, errors.New("id 为空！(id is empty!)")
	}
...
func Redeem(key string, userId int) (quota int, err error) {
	if key == "" {
		return 0, errors.New("未提供兑换码 (No redemption code provided)")
	}
	if userId == 0 {
		return 0, errors.New("无效的 user id (Invalid user ID)")
	}
	...
	err = DB.Transaction(func(tx *gorm.DB) error {
		...
		if err != nil {
			return errors.New("无效的兑换码 (Invalid redemption code)")
		}
		if redemption.Status != common.RedemptionCodeStatusEnabled {
			return errors.New("该兑换码已被使用 (This redemption code has already been used)")
		}
	...
	if err != nil {
		return 0, errors.New("兑换失败，(Redemption failed,) " + err.Error())
	}
...
func DeleteRedemptionById(id int) (err error) {
	if id == 0 {
		return errors.New("id 为空！(id is empty!)")
	}
	redemption := Redemption{Id: id}
